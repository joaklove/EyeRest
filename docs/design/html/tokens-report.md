...
- 产出：`tokens.css`（69 变量，`:root` 作用域）
        `tokens.wxss`（同 69 变量，`page` 作用域）
...
- **CSS 侧：零损耗。** 69 个变量全部直出，含 3 个阴影——因为 `tokens.py` 里的阴影本来就是 CSS 语法。
- **WXSS 侧：接近零损耗。** 只需把作用域从 `:root` 换成 `page`，并逐文件 `@import`。
- **QSS 侧：有损。** 变量机制不存在，阴影与模糊能力缺失，这套 token 需要写一个 Qt 专用转换器把值代入 QSS 字面量。

> 具体而言：`tokens.py` 的 69 个 token 里，`shadow.*` 3 个在 QSS 侧无处安放（必须改用 `QGraphicsDropShadowEffect`，而它一个 widget 只能挂一个）；`font-size.*` 的 7 个要写进 QSS `font-size`；其余颜色/间距/圆角可以代入字面值使用，但**失去「改一处全变」的能力**。
